...
public class SpiderController : MonoBehaviour
{
    ...
    public float maxTiltAngle = 45f;  // 최대 기울기 각도 설정
      ...
      /*  float valueY = Input.GetAxis("Vertical");
       ...
       / float valueX = Input.GetAxis("Horizontal");
        ...
        if (valueX != 0 || valueY != 0)
        {
            ...
            // 최대 기울기 각도 제한
            float tiltAngle = Vector3.Angle(upward, Vector3.up);
            if (tiltAngle > maxTiltAngle)
            {
                upward = Vector3.Lerp(upward, Vector3.up, (tiltAngle - maxTiltAngle) / maxTiltAngle);
            }
