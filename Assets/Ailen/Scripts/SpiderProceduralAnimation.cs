...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        // z 축 회전 값이 제한 값을 넘어가는지 확인
        if (rotationEulerAngles.z > rotationLimit && rotationEulerAngles.z < 360f - rotationLimit)
        {
            // 제한 값을 넘어가면 회전 값 수정
            rotationEulerAngles.z = rotationLimit;
            transform.rotation = Quaternion.Euler(rotationEulerAngles);
        }
        else if (rotationEulerAngles.z < 360f - rotationLimit && rotationEulerAngles.z > 0f + rotationLimit)
        {
            // 제한 값을 넘어가면 회전 값 수정
            rotationEulerAngles.z = 360f - rotationLimit;
            transform.rotation = Quaternion.Euler(rotationEulerAngles);
        }
